...
const borrowingData = [
  {
    ...
    departement: "PENGADAAN DAN TEKNOLOGI INFORMASI",
  },
  {
    id: "01112",
    tanggal: "05/01/2025",
...
export default function History() {
  ...
  const handleDelete = (id) => {
    if (confirm("Hapus history ini?")) {
      alert("Data dihapus (placeholder)");
    }
  };
        ...
        <div className="flex-1 flex justify-center">
          <div className="relative w-full max-w-lg">
            <input
              ...
              className="w-full pl-5 pr-12 py-3 rounded-full border border-gray-300 bg-white text-sm shadow-sm
        ...
        <div className="flex items-end gap-2 mb-0 px-2">
          {['adding', 'borrowing', 'returning'].map((tab) => (
            <button
              ...
              className={`px-6 py-3 rounded-t-xl font-semibold text-sm transition-all duration-200 capitalize
                ${activeTab === tab 
                  ? "bg-gradient-to-r from-blue-600 to-blue-700 text-white shadow-lg z-10" 
                  : "bg-gray-200 text-gray-500 hover:bg-gray-300" 
                }`}
            >
              {tab === 'adding' ? 'History of Adding' : 
               tab === 'borrowing' ? 'History of Borrowing' : 'History of Returning'}
            </button>
          ))}
        </div>
        ...
        <div className="bg-white rounded-b-xl rounded-tr-xl shadow-lg border border-gray-200 overflow-hidden min-h-[500px]">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full">
              ...
              {/* HEADER TABEL - SEMUA DIBUAT text-center */}
              <thead className="bg-slate-700 text-white text-sm uppercase tracking-wider">
                <tr>
                  <th className="px-6 py-4 text-center font-semibold">
                    {activeTab === 'adding' ? 'Tanggal Pembelian' : 
                     activeTab === 'borrowing' ? 'Tanggal Peminjaman' : 'Tanggal Pengembalian'}
                  </th>
                  ...
                  {/* Kolom Khusus 'Borrowing' & 'Returning' */}
                  {(activeTab === 'borrowing' || activeTab === 'returning') && (
                    <>
                      ...
                      <th className="px-6 py-4 text-center font-semibold">
                        {activeTab === 'borrowing' ? 'Surat Peminjaman' : 'Surat Pengembalian'}
                      </th>
                    </>
                  )}
              ...
              {/* BODY TABEL - SEMUA DIBUAT text-center */}
              <tbody className="divide-y divide-gray-100">
                {filterData(
                  activeTab === 'adding' ? addingData : 
                  activeTab === 'borrowing' ? borrowingData : returningData
                ).map((row, index) => (
                  <tr 
                    key={index} 
                  ...
                  >
                      ...
                      <>
                        ...
                        <td className="px-6 py-4 text-center">
                          <button 
                             ...
                             onClick={() => alert("Lihat Detail Kwitansi")}
                          >
                             Lihat Detail
                          </button>
                        </td>
                    ...
                    {/* Isi Khusus 'Borrowing' & 'Returning' */}
                    {(activeTab === 'borrowing' || activeTab === 'returning') && (
                      <>
                        ...
                        {/* PERBAIKAN: Hapus styling kotak, jadi teks biasa & Rata Tengah */}
                        <td className="px-6 py-4 text-sm text-center text-gray-700 font-medium uppercase">
                          {row.departement}
                        </td>
                        ...
                        <td className="px-6 py-4 text-center">
                          <button 
                             className="text-blue-600 hover:text-blue-800 font-medium text-sm"
                             onClick={() => alert("Lihat Detail Surat")}
                          >
                             Lihat Detail
                          </button>
                        </td>
          ...
          <div className="p-4 border-t border-gray-200 bg-gray-50 flex justify-end">
             <div className="flex items-center gap-2">
                <button className="p-2 rounded-lg hover:bg-gray-200 disabled:opacity-50 text-gray-600" disabled>
