...
function MasterData() {
  ...
  const statusData = [
    {
      label: "Bagus",
      value: kondisiSummary.total
        ? Math.round((kondisiSummary.bagus / kondisiSummary.total) * 100)
        : 0,
      color: "#38BDF8",
    },
    {
      label: "Diperbaiki",
      value: kondisiSummary.total
        ? Math.round((kondisiSummary.diperbaiki / kondisiSummary.total) * 100)
        : 0,
      color: "#2563EB",
    },
    {
      label: "Rusak",
      value: kondisiSummary.total
        ? Math.round((kondisiSummary.rusak / kondisiSummary.total) * 100)
        : 0,
      color: "#EF4444",
    },
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
        {filteredItems.slice(0, visibleCount).map((item) => {
          ...
          return (
            <div
              key={item.id}
              className="group bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-xl transition-all relative"
            >
